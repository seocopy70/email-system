...
export const api = {
  ...
  meta: () => req<{ presets: Record<string, any>; var_tags: { label: string; tag: string }[] }>("/api/meta"),
  ...
  preview: (body: any) => req<{ subject: string; html: string }>("/api/preview", { method: "POST", body: JSON.stringify(body) }),
  ...
  send: (body: any) => req<{ sent: number; skipped: number; failed: number; errors: string[] }>("/api/send", {
    method: "POST",
